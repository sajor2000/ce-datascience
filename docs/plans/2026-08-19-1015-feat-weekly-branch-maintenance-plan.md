...
- KTD4. **Discover the default branch and require matching merged pull-request evidence.** Read the repository's configured default branch rather than assuming `main`, then require the current branch SHA to equal the head SHA of at least one merged pull request. This rejects post-merge activity without discarding valid merge evidence when a later pull request closed unmerged; default, protected, and open-PR branches remain excluded.

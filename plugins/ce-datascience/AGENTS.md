...
These instructions apply when working under `plugins/ce-datascience/`.
They supplement the repo-root `AGENTS.md`.

# CE DataScience Plugin Development
...
**All skills and agents** use the `ce-` prefix to identify them as ce-datascience components:
- `/ce-brainstorm` - Explore study design and research questions before planning
- `/ce-plan` - Create SAPs or implementation plans
- `/ce-code-review` - Statistical, methodological, and code review
- `/ce-work` - Execute analysis tasks with SAP tracking
- `/ce-compound` - Document validated analytical approaches and methods

**Why `ce-`?** Claude Code has built-in `/plan` and `/review` commands. The `ce-` prefix makes it immediately clear these components belong to this plugin. The hyphen is used instead of a colon to avoid filesystem issues on Windows and to align directory names with frontmatter names.

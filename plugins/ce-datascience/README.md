...
- Codex should use native plugin install for skills plus the Bun `--to codex` agent bridge for generated agents. Set `CODEX_HOME` when installing into a non-default Codex profile.
...
- Pi, Gemini CLI, OpenCode, and Kiro are generated by the shared converter and should be validated through temp-root installs before release.

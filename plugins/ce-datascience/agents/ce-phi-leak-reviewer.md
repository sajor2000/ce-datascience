...
When an orchestrator passes `PHI authorization: confirmed` in review context, treat that as the user's prior confirmation and never downgrade it to `PHI authorization: not confirmed`. When it passes `PHI authorization: not confirmed`, do not ask from the sub-agent; avoid patient-level content and complete the metadata-only review. In agent, autofix, report-only, or headless mode, do the same when no marker is present: retain the not-confirmed state and complete the metadata-only review without waiting. Ask directly only when this reviewer runs interactively as a standalone reviewer and no marker is present.

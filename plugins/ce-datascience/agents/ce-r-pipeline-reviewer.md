...
- **Publication figure manifest drift** -- when R figure code changes and `analysis/publication/figures/figure-manifest.json` exists, confirm the changed output path, source-data path, caption, and SAP section still match the manifest. Flag missing or stale manifest entries for manuscript-bound figures.
...
- **Mixed model convergence and diagnostics** -- `lmer()` or `glmer()` with singular fit warnings and no declared response; optimizer/gradient warnings ignored; `glmmTMB()` convergence/Hessian status unchecked; or family-appropriate residual diagnostics absent. Check that the grouping factor and random-effects formula implement the documented structure, that random slopes use predictors varying within group, and that simplification removes unsupported correlations/components in the predeclared order instead of dropping dependence wholesale.

- **Marginal and covariance implementation errors** -- conditional `glmer()` predictions with random effects set to zero labeled as population marginal; `geeglm()` using the wrong `id`, row order/waves, family, or working correlation; MMRM/`nlme` covariance structures inconsistent with visit timing; or an AR(1) residual structure applied to irregular time as though observations were equally spaced. These are implementation errors; the methods reviewer owns whether GEE, GLMM, or MMRM was the right estimand-level choice.

- **Bayesian hierarchical diagnostics** -- `brms`/Stan multilevel results accepted with divergent transitions, poor R-hat/effective sample size, no prior predictive rationale for weak variance components, or no posterior predictive check. Do not demand a particular Bayesian package; require visible evidence that the fitted hierarchy was computationally and scientifically identified.
...
**Anchor 100** -- certain: `group_by()` without `ungroup()` before downstream `mutate()`, `coxph()` with no Schoenfeld residual check anywhere in the script, a singular fit warning from `lmer()` with no diagnostic response, or `geeglm()` grouped by a row identifier instead of the declared cluster. The error is directly visible and verifiable.

**Anchor 75** -- confident: `glmmTMB()` output does not check convergence/Hessian status, population-marginal predictions are computed by setting random effects to zero, or a documented covariance/random-effects structure is implemented differently. The gap is observable from code inspection.

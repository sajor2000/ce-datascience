...
| 4 | Participants | Setting, eligibility, treatment received | ☐ | use the `ce-cohort-build` skill |
| 5 | Outcome | Outcome to be predicted, including how/when assessed | ☐ | |
| 6 | Predictors | Predictors used, how/when measured | ☐ | |
| 7 | Sample size | How the study size was determined | ☐ | use the `ce-power` skill |
...
| 12 | Algorithmic fairness | Performance by sex, race, age, hospital, payer | ☐ | use the `ce-fairness-reviewer` skill |
| 13 | Calibration | Calibration plot, Brier score, ICI | ☐ | use the `ce-calibration-reviewer` skill |
| 14 | Dataset shift | Plan for monitoring or detecting concept drift | ☐ | use the `ce-concept-drift-reviewer` skill |
| 15 | Discrimination | C-statistic, AUC with CI; comparison to existing models | ☐ | |
| 16 | Model card | Mitchell-style model card published with the study | ☐ | use the `ce-model-card` skill |
...
| 23 | Data leakage check | Statement that no train-test contamination occurred | ☐ | use the `ce-data-leakage-reviewer` skill |

...
| `ce-clif-reviewer` | `__CE_CLIF__ active=true` in chat context OR repo signals CLIF (mCIDE/, WORKFLOW.md, CLIF_CLAUDE.md, clif-icu / clif-consortium remote). Block any diff that touches mCIDE/**, ddl/**, outlier-handling/**, reference_ranges/**, or WORKFLOW.md without a `POC: @<handle> approved` token in the PR description (see `ce-clif/references/poc-table.md`). Block CSV/Feather IO for CLIF tables, tz-naive `*_dttm`, integer cast of patient_id/hospitalization_id, and unknown `*_category` strings (validate against `ce-clif/references/mcide-vocab.md`). Block patient-level rows written to `output/`. |

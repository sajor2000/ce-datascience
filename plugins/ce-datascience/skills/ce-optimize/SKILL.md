...
These guardrails apply to every optimization run in this plugin. They are not optional.

1. **Cross-validation awareness.** Never optimize on the test set. If the experiment modifies model hyperparameters, feature sets, or prediction thresholds, each iteration must evaluate via cross-validation on the training set (or a held-out validation set). The test set is used only for the final locked result after optimization converges.

2. **Data leakage guard.** Before each experiment, verify that feature engineering does not use the outcome variable, that normalization/encoding is fit on training data only, and that temporal ordering is preserved for time-series splits. If a leakage signal is detected, abort the experiment with a FAIL verdict rather than recording a misleading metric.

3. **Experiment tracking integration.** When `ce-ml-experiment-track` has been configured (mlflow, wandb, dvc, or offline-YAML), log each experiment iteration to the configured backend. Record: iteration number, hypothesis, hyperparameters changed, metric values, and outcome (kept/reverted).

4. **SAP alignment check.** If a SAP file exists (`docs/plans/` with `sap_version` in frontmatter), read the SAP's primary endpoint and verify the optimization target metric aligns with it. If the optimization target differs from the pre-registered primary endpoint, surface a warning before starting and require explicit user confirmation to proceed.
...
The files under `.context/ce-datascience/ce-optimize/<spec-name>/` are local scratch state. They are ignored by git, so they survive local resumes on the same machine but are not preserved by commits, branches, or pushes unless the user exports them separately.
...
### File Locations (all under `.context/ce-datascience/ce-optimize/<spec-name>/`)
...
6. Write the spec to `.context/ce-datascience/ce-optimize/<spec-name>/spec.yaml`
...
**If branch exists**, check for an existing experiment log at `.context/ce-datascience/ce-optimize/<spec-name>/experiment-log.yaml`.
...
```bash
mkdir -p .context/ce-datascience/ce-optimize/<spec-name>/
```
...
1. Create the experiment log file at `.context/ce-datascience/ce-optimize/<spec-name>/experiment-log.yaml`
...
7. **IMMEDIATELY append to experiment log on disk (CP-3)** — do not defer this to batch evaluation. Write the experiment entry (iteration, hypothesis, outcome, metrics, learnings) to `.context/ce-datascience/ce-optimize/<spec-name>/experiment-log.yaml` right now. Use the transitional outcome `measured` once the experiment has valid metrics but has not yet been compared to the current best. Update the outcome to `kept`, `reverted`, or another terminal state in the evaluation step, but the raw metrics are on disk and safe from context compaction.
...
4. **Write strategy digest** to `.context/ce-datascience/ce-optimize/<spec-name>/strategy-digest.md`:
...
rm -f .context/ce-datascience/ce-optimize/<spec-name>/strategy-digest.md
```

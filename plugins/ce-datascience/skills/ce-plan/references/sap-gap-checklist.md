...
- [ ] **Unresolved causal/observational analysis assumptions** -- For observational studies or causal claims, state the estimand, causal assumptions, unit/grain, key fields, time zero, and success criteria. Choices that would change the estimand or interpretation must be resolved or documented as analyst-approved assumptions before finalization.
- [ ] **No tabular SAP workbook contract** -- Every new SAP must include or explicitly require the biostatistics-style tabular companion (`analysis/sap-tables/01-overview.csv`, `02-outputs.csv`, `03-variables.csv`, and `<slug>-tabular-sap.xlsx` when available). If data QA is missing, the SAP may remain draft, but it must name `/ce-data-qa` then `/ce-sap-tabular <slug>` as required next steps before sprint/work.

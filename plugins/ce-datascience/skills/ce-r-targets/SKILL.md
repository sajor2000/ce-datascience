...
4. Validate target inputs with `/ce-data-qa`; a successful pipeline run does not prove source integrity.
5. Run the narrowest affected target, then the project's normal pipeline/manifest command. Report invalidated targets and unexplained cache behavior.

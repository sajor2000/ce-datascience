...
Data comes from the GitHub Releases API for `sajor2000/ce-datascience`, filtered to the `ce-datascience-v*` tag prefix so sibling components (`cli-v*`, `marketplace-v*`, `cursor-marketplace-v*`) are excluded.
...
Version-like inputs (`2.65.0`, `v2.65.0`, `ce-datascience-v2.65.0`) are query strings, not a separate lookup-by-version mode. They flow through query mode like any other text.
...
> `python3` is required to load the `ce-release-notes` skill. Install Python 3.x and retry, or open https://github.com/sajor2000/ce-datascience/releases directly.
...
```json
{
  ...
  "fetched_at": "...",
  "releases": [
    {"tag": "ce-datascience-v2.67.0", "version": "2.67.0", "name": "...",
...
If `ok: true`, take the first 5 entries from `releases` (the helper has already filtered to `ce-datascience-v*` and sorted newest first). If fewer than 5 are available, render whatever count came back without warning.
...
Browse all releases at https://github.com/sajor2000/ce-datascience/releases
```
...
```bash
gh pr view <linked_prs[0]> --repo sajor2000/ce-datascience --json title,body,url
```
...
```
I couldn't find this in the last 40 plugin releases. Browse the full history at https://github.com/sajor2000/ce-datascience/releases
```

...
1. **Read Plan and Clarify** _(skip if arriving from Phase 0 with a bare prompt)_
   ...
   - **CLIF discovery:** When chat context contains `__CE_CLIF__ active=true` (or the repo has strong CLIF signals: `CLIF_CLAUDE.md` at root, or `clif-icu`/`clif-consortium` git remote — do NOT activate from `mCIDE/` or `WORKFLOW.md` alone as these appear in non-CLIF projects), apply the CLIF guardrails throughout execution: refuse edits to `mCIDE/**`, `ddl/**`, `outlier-handling/**`, `reference_ranges/**`, `WORKFLOW.md` unless the prompt or PR description contains `POC: @<handle> approved` (resolve `<handle>` from `ce-clif/references/poc-table.md`); refuse CSV/Feather IO for CLIF tables (Parquet only); enforce timezone-aware UTC `*_dttm`; preserve VARCHAR `patient_id` / `hospitalization_id`; validate `*_category` writes against `ce-clif/references/mcide-vocab.md`; never write patient-level rows to `output/`; place new analysis files under the three-script architecture (`code/01_qc_*`, `code/02_cohort_*`, `code/03_analysis_*`). For code, route by language: `__CE_LANG__ primary=python` -> `ce-clif/references/clifpy-recipes.md` (use `ClifOrchestrator`); `__CE_LANG__ primary=r` -> `ce-clif/references/r-template-recipes.md` (use the `CLIF-Project-Template` layout). If `__CE_LANG__` is absent, run `/ce-language-detect` first; if still `unknown`, surface both recipe files. Print one banner before the task list: `[ce-clif] CLIF profile active; protected paths read-only without POC sign-off.`

---
name: ce-work
description: "Execute implementation or analysis plans with task tracking, tests, SAP awareness, and shipping checks."
...
- **Problem it solves:** Plans need disciplined execution that respects scope, tests behavior, and finishes complete work.
- **Use when:** The user asks to work, implement, execute a plan, or continue from the latest plan document.
- **Output:** Implemented changes, passing validation, updated task status, and shipping-ready summary.
- **Ask only if:** Only when plan scope, branch choice, or execution-blocking ambiguity cannot be resolved from the plan and repo.
- **Do not do:** Do not edit plan progress state, skip tests for behavior changes, or commit to default branch without permission.
...
1. **Read Plan and Clarify** _(skip if arriving from Phase 0 with a bare prompt)_
   ...
   - If the plan creates or materially extends a Marimo, Jupyter, R Markdown, or Quarto notebook, load the `ce-notebook-standards` skill before creating executable cells or chunks.
   ...
   - **Data QA gate before coding/modeling:** If a SAP file is found, inspect it for `<!-- GAP: missing ce-data-qa column profile` or any missing data-profile/QA prerequisite. If present, stop execution and route to the `ce-data-qa` skill before opening `ce-sprint`, scaffolding code, or running models. If the SAP has no gap comment, still require a `reports/data-qa/*.md`, `analysis/data-qa-report.md`, `__CE_DATA_PROFILE__`, or `__CE_DATA_QA__` signal before executing any task that reads data, fits models, generates tables/figures, or computes study results. If no data exists yet, restrict work to data acquisition/profile tasks and refuse modeling until QA evidence exists.
   - **Real-data execution gate:** Before any notebook, script, or interactive run that reads, creates, transforms, validates, or renders analytical data or output, resolve the declared restricted data root for file-backed data (or the registered verified data connection) and confirm its provenance against the QA evidence. Do not generate or substitute synthetic, mock, sample, or fake-study data unless the user explicitly requests a clearly labeled test or demonstration. If the real source is unavailable or provenance cannot be confirmed, do not run the analytical work and do not persist output; report the run as blocked. Persisted study artifacts must identify their real source location or registered extract ID/hash and the validation evidence used. An explicitly requested synthetic test/demo may persist only as a clearly labeled non-study artifact with synthetic fixture or generator provenance, never in a study-output location or as a study result.
   - **Tabular-catalog discovery:** Also check for `analysis/sap-tables/02-outputs.csv` (produced by `ce-sap-tabular`). When the catalog exists, seed the task list with one task per output row in the catalog, excluding section banner rows (rows where only the first cell is populated) and legacy section-header rows. Accept both the current biostatistics workbook labels and legacy normalized labels: output file = `Output File (SITE_ID_ prefix added automatically)` or `output_file`; location = `Subfolder` or `subfolder`; owning analysis = `Script Section` or `analysis_section`; expected meaning = `Interpretation` or `interpretation`. Refuse to mark a catalog-driven task complete until the expected output file exists at the expected subfolder path. When the catalog does not exist, skip this step; behavior reverts to the prose SAP tracking above.
   - **Sprint discovery:** Read `analysis/sprint-log.yaml` if present. If a sprint has `status: open`, the run is happening inside a bounded analysis sprint and that sprint's `scope` list is a hard scope filter — refuse edits to files whose owning SAP section is not in the open sprint's scope, and refuse to add tasks outside scope. Print a one-line banner before the task list: `[sprint:<name> reviewer:<reviewer> scope:<sap-ids>]`. The banner makes the sprint context visible so the user knows why scope is narrower than the SAP. When no sprint is open (status absent, closed, or no sprint-log.yaml), behavior is unchanged.
   - **CLIF discovery:** When chat context contains `__CE_CLIF__ active=true` (or the repo has strong CLIF signals: `CLIF_CLAUDE.md` at root, or a git remote containing `clif-icu`, `clif-consortium`, or `Common-Longitudinal-ICU-data-Format` — do NOT activate from `mCIDE/` or `WORKFLOW.md` alone as these appear in non-CLIF projects), apply the CLIF guardrails throughout execution. Treat `https://clif-icu.com/` plus matching handoff `version=` and `mcide_version=` values as the source of truth. If the family is not selected, load the `ce-clif` skill before emitting category filters or category validation; never assume a 2.1 cache or infer 3.0 from an `mCIDE/` path. Refuse edits to `mCIDE/**`, `ddl/**`, `outlier-handling/**`, `reference_ranges/**`, `WORKFLOW.md` unless the prompt or PR description contains `POC: @<handle> approved`; if the POC cannot be resolved from local project docs, block and ask for the approving CLIF contact instead of guessing. Before opening patient-level data or outputs, check whether the user already confirmed in the current conversation that both the data environment and active model endpoint are approved for PHI/PII. If confirmed, proceed without asking again or repeating that PHI cannot be read. If absent or ambiguous in interactive mode, ask once and wait; if the answer is no, work from schemas, code, and reviewed aggregates. In agent, autofix, report-only, or headless mode, do not ask or wait; retain `PHI authorization: not confirmed` and continue only from paths, code, schemas, metadata, and reviewed aggregates. Regardless of authorization, do not reproduce PHI in responses or persist it in Git, logs, screenshots, issues, or unrestricted exports. Refuse CSV/Feather IO for CLIF tables (Parquet only); enforce timezone-aware UTC `*_dttm`; preserve VARCHAR `patient_id` / `hospitalization_id`; validate `*_category` writes against the selected mCIDE family. Keep patient-level working artifacts under the declared approved restricted data root; use `output/intermediate_phi/` only when that exact path is approved, local, non-synced, and gitignored. Write shareable aggregates only to `output/final_no_phi/`. Place new analysis files under cohort → quality checks → outlier handling → analysis (`code/01_*` through `code/04_*`). For code, route by language: `__CE_LANG__ primary=python` -> use `clifpy` / `ClifOrchestrator`; `__CE_LANG__ primary=r` -> use the CLIF project-template layout with `arrow::open_dataset()`. If `__CE_LANG__` is absent, load the `ce-language-detect` skill first; if still `unknown`, surface both implementation choices. Print one banner before the task list: `[ce-clif] CLIF profile active; protected paths read-only without POC sign-off.`
...
1. **Task Execution Loop**
   ...
   **Fail-loud analytical execution:** For generated analytical transformations, do not swallow exceptions with bare catch-all handlers, do not invent fallback data, do not silently coerce values, and do not replace failed inputs with synthetic data. Synthetic data are allowed only when the user explicitly requests a clearly labeled test or demonstration; never produce a synthetic study result. Assert validation at critical input, join, transformation, and output boundaries; surface failures for diagnosis rather than masking them, without expanding the requested analysis scope.

...
1. **Read Plan and Clarify** _(skip if arriving from Phase 0 with a bare prompt)_
   ...
   - **Tabular-catalog discovery:** Also check for `analysis/sap-tables/02-outputs.csv` (produced by `/ce-sap-tabular`). When the catalog exists, seed the task list with one task per row in the catalog (excluding section header rows). Each task takes the row's `output_file`, `subfolder`, `analysis_section`, and `interpretation` as the task's expected output, location, owning analysis, and pre-registered direction. Refuse to mark a catalog-driven task complete until the expected `output_file` exists at the expected `subfolder` path. When the catalog does not exist, skip this step; behavior reverts to the prose SAP tracking above.

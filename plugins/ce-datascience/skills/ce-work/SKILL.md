...
4. **Choose Execution Strategy**
   ...
   **Subagent isolation** — give each parallel subagent its own working tree:
   - **Claude Code (`Agent` tool):** pass `isolation: "worktree"` and `run_in_background: true`. The harness creates a per-subagent worktree under `.claude/worktrees/agent-<id>` on its own branch. Verify `.claude/worktrees/` is gitignored before relying on this. Dispatch with the intended base commit SHA; before editing, the worker verifies its `HEAD` equals that SHA and stops on mismatch. A unit that depends on uncommitted state cannot use an isolated snapshot.

...
If no config exists, load the `ce-language-detect` skill detection rules (check for `renv.lock`, `requirements.txt`, `pyproject.toml`, `.R` files, `.py` files) to infer the primary language.

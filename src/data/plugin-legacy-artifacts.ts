...
const EXTRA_LEGACY_ARTIFACTS_BY_PLUGIN: Record<string, LegacyPluginArtifacts> = {
  ...
  "ce-datascience": {
    // Mirrors the compound-engineering entry for installs under the current
    // plugin name. Keep both entries so legacy installs are still cleaned up.
    skills: [
      "agent-browser",
      "agent-native-architecture",
      "agent-native-audit",
...
export function getLegacyCodexArtifacts(bundle: CodexBundle): LegacyTargetArtifacts {
  ...
  // CE skill or agent would be swept into `ce-datascience/legacy-backup`
...
export function getLegacyWindsurfArtifacts(plugin: ClaudePlugin): LegacyWindsurfArtifacts {
  ...
  // swept into `ce-datascience/legacy-backup` even though it was never
  // installed by CE.
  //

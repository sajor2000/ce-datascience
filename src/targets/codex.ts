...
const PREV_CE_START_MARKER = "# BEGIN Compound Engineering plugin MCP -- do not edit this block"
const PREV_CE_END_MARKER = "# END Compound Engineering plugin MCP"
...
export async function writeCodexBundle(
  ...
  // Codex uses the same hooks shape as Claude Code. Keep plugin-owned hooks
...
export function mergeCodexConfig(existingContent: string, mcpToml: string | null): string | null {
  ...
  for (const [start, end] of [[MANAGED_START_MARKER, MANAGED_END_MARKER], [PREV_START_MARKER, PREV_END_MARKER], [PREV_CE_START_MARKER, PREV_CE_END_MARKER]]) {

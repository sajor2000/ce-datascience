...
//
// The sibling registry `EXTRA_LEGACY_ARTIFACTS_BY_PLUGIN["ce-datascience"].skills`
...
// now installs at `~/.codex/skills/ce-datascience/ce-update` but had a
// pre-namespaced flat install at `~/.codex/skills/ce-update`). Sweeping the
// flat path is correct even while the skill is current.

const PLUGIN_ROOT = path.join(import.meta.dir, "..", "plugins", "ce-datascience")
